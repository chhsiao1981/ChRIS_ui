...
const title = "Rename Data";
const label = "New name";
const buttonLabel = "Rename";
...
export default (props: Props) => {
  ...
  const selected = selectedPaths.length ? selectedPaths[0] : null;
  ...
  useEffect(() => {
    if (modalState.type !== "rename" || !selected) {
      return;
    }
  ...
  const onSubmit = (value: string) => {
    console.info("RenameModal.onSubmit: to doCart.rename: value:", value);
    doCart.rename(cartID, value);
    setValue("");
    doOperation.closeModal(operationID);
  };
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div>
            ...
            <div style={{ minHeight: "20px", marginTop: "2px" }}>
              <HelperText>
                <HelperTextItem>You can rename this data.</HelperTextItem>
              </HelperText>
            </div>
          </div>
